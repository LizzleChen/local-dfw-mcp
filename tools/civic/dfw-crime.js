...
import { SODA, ARCGIS, requireVerified } from "../../lib/sources.js";
 ...
 * layer, live-verified 2026-07-14) alongside the original Dallas path. Default
 * behavior when `city` is omitted is UNCHANGED (Dallas, with the v0.1
 * auto-detect guard) -- Fort Worth only fires on an explicit `city:"fortworth"`.
 *
...
export const dfwCrime = {
  ...
  description: withAttributionTag(
    "City of Dallas (default) or Fort Worth (city=\"fortworth\", added v0.2) -- " +
      "no other city wired. Search police incidents by (block-level) address " +
      "and/or offense type. Returns incident number, offense, date, premise " +
      "(Dallas) or location type (Fort Worth), division/sector/beat, and " +
      "status. Addresses are block-level (privacy-rounded upstream). NOT a " +
      "consumer report — do not use for tenant, employment, or other " +
      "FCRA-regulated screening. Sources: Dallas Police Department Open Data / " +
      "City of Fort Worth Police Crime Data."
  ),
  inputSchema: {
    ...
    city: z.enum(["dallas", "fortworth", "auto"]).optional()
      .describe('Jurisdiction: "dallas" (default) queries Dallas PD (Socrata); "fortworth" (v0.2) queries Fort Worth PD Crime Data (ArcGIS); "auto" (Dallas only) resolves Dallas-vs-elsewhere from the address, unchanged from v0.1. Fort Worth is used ONLY when city="fortworth" is passed explicitly -- an address that geocodes into Fort Worth without that flag is still refused as "not covered" by the Dallas path.'),
