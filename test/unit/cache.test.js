...
test("cached: concurrent callers share the in-flight promise", async () => {
  clearAll();
  let calls = 0;
  const load = async () => { calls++; await new Promise((r) => setTimeout(r, 20)); return "x"; };
...
test("cached: loader errors are NOT cached", async () => {
  clearAll();
  let calls = 0;
  await assert.rejects(cached("k4", 60_000, async () => { calls++; throw new Error("nope"); }), /nope/);
  const value = await cached("k4", 60_000, async () => { calls++; return "ok"; });
  assert.equal(value, "ok");
  assert.equal(calls, 2);
});

test("cached: respects DFW_CACHE_DISABLED=1", async () => {
  clearAll();
