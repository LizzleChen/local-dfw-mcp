...
test("withLimit: caps concurrency at provider's bucket max", async () => {
  process.env.DFW_LIMIT_TEST_BUCKET = "2";
  ...
  await Promise.all(Array.from({ length: 6 }, () => withLimit("test_bucket", work)));
...
test("withLimit: releases slot even when fn throws", async () => {
  process.env.DFW_LIMIT_ERR_BUCKET = "1";
  await assert.rejects(withLimit("err_bucket", async () => { throw new Error("boom"); }), /boom/);
  const t0 = Date.now();
...
test("withLimit: FIFO ordering of queued callers", async () => {
  process.env.DFW_LIMIT_FIFO_BUCKET = "1";
  const order = [];
